...
export function makeModuleCommand<R>(
  ...
  return Command.make('module', { path: pathArgument }, ({ path }) =>
    Effect.gen(function* () {
      const configuredPath = yield* configPath;
      const target = resolveInspectionTarget(configuredPath, path);
      const inspection = yield* inspectModule(target.configPath, target.target);
      yield* Console.log(renderModuleInspection(inspection));
    }),
  ).pipe(
