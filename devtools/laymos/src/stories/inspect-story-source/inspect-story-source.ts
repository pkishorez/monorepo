...
const localStoryModule = 'src/stories/authoring/index';
...
interface SourceAnalysis {
  ...
  readonly laymosImports: readonly SgNode[];
...
export function transformStorySource(source: string, fileName: string): string {
  if (!mayContainStoryImport(source)) return source;
...
export function validateStoryAuthoringSource(
  ...
  if (!mayContainStoryImport(source)) return [];
  try {
    const root = parse(languageFor(fileName), source).root();
    assertValidSyntax(root, fileName);
    const analysis = analyzeSource(root, fileName);
...
export function isLaymosStoryFile(source: string, fileName: string): boolean {
  if (!storyFilePattern.test(fileName) || !mayContainStoryImport(source)) {
    return false;
  }
  const root = parse(languageFor(fileName), source).root();
  assertValidSyntax(root, fileName);
  return importsFromStoryModule(root, fileName).length > 0;
}

export const planStoryEjection = flow(
  'Plan Story ejection',
  {
    description:
      'Loads project ownership and prepares every safe production rewrite before any source can change.',
  ...
  > =>
    loadConfig({ projectDir: baseDir }).pipe(
      Effect.flatMap((config) =>
        step(
          'Inspect and transform application source',
          {
            description:
              'Inventories source, excludes Module Laymos surfaces, validates authoring syntax, and builds idempotent in-memory rewrites.',
          },
          () => planStoryEjectionFiles(baseDir, config),
        ),
      ),
      Effect.mapError(normalizeEjectionError),
    ),
);

function planStoryEjectionFiles(
  baseDir: string,
  config: LaymosConfig,
): Effect.Effect<
  StoryEjectionPlan,
...
> {
  return Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const laymosSurfaces = yield* Effect.promise(() =>
      findLaymosSurfaces(baseDir, config.modules ?? []),
    );
    const excludedSurfaces = laymosSurfaces.map(({ path }) => path).sort();
    const files = yield* fs.glob(sourceFileGlob, {
      root: baseDir,
      exclude: excludedSourceGlobs,
    });
    ...
    for (const { relativePath, source } of analyzed) {
      if (
        excludedSurfaces.some((surface) => isWithinPath(relativePath, surface))
      ) {
        continue;
      }
      if (!mayContainStoryImport(source)) continue;
      try {
...
export const ejectStories = flow(
  'Eject Story authoring',
  {
    description:
      'Plans every production rewrite before choosing between a read-only report and atomic application with rollback.',
    attributes: (
      baseDir: string,
      options: { readonly dryRun?: boolean } = {},
    ) => ({ baseDir, dryRun: options.dryRun === true }),
  },
  (
    baseDir: string,
    options: { readonly dryRun?: boolean } = {},
  ...
  > =>
    planStoryEjection(baseDir).pipe(
      Effect.flatMap((plan) =>
        storyDecision(
          'Ejection is a dry run',
          {
            description:
              'Chooses whether the validated source plan may change the project.',
          },
          options.dryRun === true,
        ).pipe(
          when(
            true,
            {
              name: 'Report only',
              description:
                'Return the changed paths without writing planned content.',
              completion: { kind: 'success' },
            },
            () =>
              terminal(
                'Dry-run plan reported',
                {
                  description:
                    'Completes with the files that would change while leaving source untouched.',
                  completion: { kind: 'success' },
                },
                () =>
                  Effect.succeed({
          ...
          when(
            false,
            {
              name: 'Apply',
              description:
                'Write the complete plan and restore earlier files if any later write fails.',
              errors: ['StoryEjectionError'],
            },
            () => applyEjectionPlan(baseDir, plan),
          ),
          exhaustive,
        ),
      ),
...
const applyEjectionPlan = flow(
  'Apply ejection plan',
  {
    description:
      'Writes the validated plan and makes successful commit or project restoration explicit.',
    attributes: (baseDir: string, plan: StoryEjectionPlan) => ({
      baseDir,
  ...
  > =>
    Effect.gen(function* () {
      ...
      const writeExit = yield* step(
        'Write planned source atomically',
        {
          description:
            'Writes through temporary sibling files and remembers completed rewrites for possible restoration.',
        },
        () =>
          writeEjectionPlan(fs, path, baseDir, plan, written).pipe(Effect.exit),
      );
      return yield* storyDecision(
        'All planned writes succeeded',
        {
          description:
            'Chooses between the completed ejection and restoring every file changed by this attempt.',
        ...
        when(
          true,
          {
            name: 'Committed',
            description:
              'Return every changed path after all atomic writes complete.',
            completion: { kind: 'success' },
          },
          () =>
            terminal(
              'Story authoring was ejected',
              {
                description:
                  'Completes with the applied production source changes.',
                completion: { kind: 'success' },
              },
              () =>
                Effect.succeed({
        ...
        when(
          false,
          {
            name: 'Restore',
            description:
              'Restore every completed rewrite before returning the write failure.',
            errors: ['StoryEjectionError'],
          },
          () =>
            step(
              'Restore files changed by this attempt',
              {
                description:
                  'Writes remembered original source back atomically before the operation fails.',
              },
              () => restoreRewrites(fs, path, baseDir, written),
            ).pipe(
              Effect.andThen(
                terminal(
                  'Ejection was rolled back',
                  {
                    description:
                      'Returns a typed write failure only after restoring the project.',
                    completion: {
                      kind: 'error',
                      error: 'StoryEjectionError',
                    },
                  },
                  () =>
                    Effect.fail(
                      new StoryEjectionError({
                        message: `Story ejection could not apply source changes: ${writeFailure(writeExit)}`,
                      }),
                    ),
                ),
              ),
...
function writeEjectionPlan(
  fs: FileSystem.FileSystem,
  path: Path.Path,
  baseDir: string,
  plan: StoryEjectionPlan,
  written: StoryEjectionRewrite[],
...
function writeFailure(exit: Exit.Exit<unknown, unknown>): string {
  return Exit.isFailure(exit) ? String(Cause.squash(exit.cause)) : 'unknown';
}

function normalizeEjectionError(error: unknown): StoryEjectionError {
  return error instanceof StoryEjectionError
    ? error
    : new StoryEjectionError({ message: String(error) });
}
...
function analyzeSource(root: SgNode, fileName: string): SourceAnalysis {
  ...
  const laymosImports = importsFromStoryModule(root, fileName);
  const unsupported: string[] = [];

  for (const statement of laymosImports) {
  ...
  for (const statement of root.findAll(byKind('export_statement'))) {
    if (!isStoryModule(moduleSource(statement), fileName)) continue;
    if (
  ...
  for (const call of root.findAll(byKind('call_expression'))) {
    ...
    if (
      args[0] !== undefined &&
      isStoryModule(stringValue(args[0]), fileName)
    ) {
      unsupported.push(
...
function renderNode(
  ...
  if (
    ...
    analysis.laymosImports.some((item) => item.id() === node.id())
  ) {
...
function renderStoryImport(node: SgNode): string {
  ...
  return `import { ${remaining.join(', ')} } from ${JSON.stringify(moduleSource(node) ?? storyModule)};`;
}
...
function storyReferences(
  ...
  const importNodeIds = new Set(
    analysis.laymosImports.flatMap((statement) =>
...
function importsFromStoryModule(
  root: SgNode,
  fileName: string,
): readonly SgNode[] {
  return root
    .findAll(byKind('import_statement'))
    .filter((statement) => isStoryModule(moduleSource(statement), fileName));
}

function isStoryModule(
  moduleName: string | undefined,
  fileName: string,
): boolean {
  if (moduleName === storyModule) return true;
  if (moduleName === undefined || !moduleName.startsWith('.')) return false;
  const resolved = posix
    .normalize(posix.join(posix.dirname(fileName), moduleName))
    .replace(/\.(?:[cm]?[jt]sx?)$/, '');
  return resolved === localStoryModule;
}

function mayContainStoryImport(source: string): boolean {
  return source.includes(storyModule) || source.includes('authoring/index');
}
