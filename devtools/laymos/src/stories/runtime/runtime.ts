...
import { CurrentTrace, traceValue } from './trace.js';
...
export const CurrentOpaqueOperation = Context.Reference<
  OpaqueOperation | undefined
>('laymos/story/current-opaque-operation', { defaultValue: () => undefined });
...
function wrapEffect<A, E, R>(
  ...
  return Effect.gen(function* () {
    ...
    if (recorder === undefined) return yield* operation;
    const run =
      opaqueOperation === undefined
        ? operation
        : operation.pipe(
...
function makeBlock(
  ...
  requireDescription(meta.description, `${label} "${name}"`);
  return {
    name,
    kind,
