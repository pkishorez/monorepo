...
import {
  ...
  discoverTests,
  ejectStories,
  inspectStories as inspectProjectStories,
  runStories,
  runTests,
} from '../node/index.js';
import type { StoriesRunResult, StoryRunOptions } from '../node/index.js';
...
} from '../../stories/measure-story-coverage/index.js';
...
function lint(
  ...
  return validateProjectStoryAuthoring(process.cwd()).pipe(
    Effect.andThen(
      Effect.all({
        report: analyzeProject({ projectDir: process.cwd() }),
        stories: inspectStories(),
        tests: discoverTests({ projectDir: process.cwd() }),
      }),
    ),
    Effect.flatMap(({ report, stories, tests }) =>
      Effect.suspend(() => {
        reportCoverage(report);
        reportStoryLint(stories, false, verbose);
        reportTestCatalog(tests);
...
function lintTests(): Effect.Effect<void, CliExit> {
  return discoverTests({ projectDir: process.cwd() }).pipe(
    Effect.tap((catalog) => Effect.sync(() => reportTestCatalog(catalog))),
    Effect.asVoid,
    Effect.catch((error) =>
      Effect.sync(() => {
        process.stderr.write(red(`Error: ${errorMessage(error)}\n`));
...
function reportTestCatalog(catalog: TestCatalog): void {
  const tests = catalog.modules.flatMap((module) => module.tests);
  process.stdout.write(
    `Tests: ${tests.length} valid ${tests.length === 1 ? 'definition' : 'definitions'}.\n`,
  );
}
...
> {
  return inspectProjectStories({ projectDir: process.cwd() }).pipe(
    Effect.flatMap((stories) => {
      ...
      return planStoryEjection(process.cwd()).pipe(
        Effect.flatMap((ejection) =>
          Effect.try({
            try: () => measureStoryCoverage(process.cwd(), stories, ejection),
            catch: (error) => error,
          }),
        ),
...
function stories(
  ...
  return Effect.suspend(() => {
    ...
    return runStories({
      ...
      selectors: selectors.map((storyPath) => ({
        _tag: 'Story' as const,
        storyPath,
      })),
      ...options,
  ...
  return Effect.suspend(() => {
    ...
    return runTests({
      ...
      Effect.flatMap((report) => {
        const matches = reportTests(report);
        return matches ? Effect.void : Effect.fail(new CliExit());
      }),
      Effect.catch((error) =>
        error instanceof CliExit
          ? Effect.fail(error)
...
function reportTests(report: TestsReport): boolean {
  let allMatch = true;
  for (const test of Object.values(report.tests)) {
    process.stdout.write(`\n  ${test.name} (${test.testPath})\n`);
    for (const testCase of test.cases) {
      const matches = expectationsMatch(testCase.expected, testCase.actual);
      allMatch &&= matches;
      process.stdout.write(
        `    ${matches ? green('✓') : red('✗')} ${testCase.name}\n`,
      );
      process.stdout.write(
        `      Input: ${testCase.inputs.map(String).join(', ')}\n`,
      );
      process.stdout.write(
        `      Expected: ${formatExpectation(testCase.expected)}\n`,
      );
      process.stdout.write(
        `      Actual: ${formatExpectation(testCase.actual)}\n`,
      );
    }
  }
  return allMatch;
...
function expectationsMatch(
  ...
  return expected.kind === 'value' && actual.kind === 'value'
...
function formatExpectation(expectation: TestExpectation): string {
  return expectation.kind === 'value'
    ? String(expectation.value)
    : `error(${expectation.name})`;
}

function eject(
  dryRun: boolean,
): Effect.Effect<void, CliExit, FileSystem.FileSystem | Path.Path> {
  return ejectStories({ projectDir: process.cwd(), dryRun }).pipe(
    Effect.tap((result) =>
      Effect.sync(() => {
        ...
        if (result.changed.length === 0) {
          process.stdout.write('No Story instrumentation found to eject.\n');
        }
      }),
    ),
    Effect.catch((error) =>
...
function reportViolations(violations: readonly Violation[]): void {
  ...
  for (const violation of violations) {
    if (violation.kind === 'layer') {
      ...
      process.stderr.write(
        red(`  - [laymos-import] -> ${violation.to.module}\n`),
      );
    }
    process.stderr.write(
...
const lintTestsCommand = Command.make('tests', {}, () => lintTests()).pipe(
  Command.withDescription('Validate Test files and declarations.'),
);
...
const lintCommand = Command.make(
  ...
  Command.withDescription(
    'Lint architecture, Stories, and Tests. Coverage gaps are warnings only.',
  ),
  Command.withSubcommands([lintStoriesCommand, lintTestsCommand]),
);
...
const ejectCommand = Command.make(
  'eject',
  {
    dryRun: Flag.boolean('dry-run').pipe(
      Flag.withDescription('Preview rewritten files without changing them.'),
    ),
  },
  ({ dryRun }) => eject(dryRun),
).pipe(
  Command.withDescription(
    'Remove Story instrumentation from production source across the current project.',
  ),
);
...
const testsCommand = Command.make(
  'tests',
  {
    selectors: Argument.string('selector').pipe(Argument.variadic()),
    timeout: Flag.string('timeout').pipe(
      Flag.optional,
      Flag.withDescription('Default Test Case timeout, e.g. "30 seconds".'),
    ),
  },
  ({ selectors, timeout }) => tests(selectors, timeout),
).pipe(
  Command.withDescription(
    'Run Module paths or suffixless Test paths (all Tests by default) and print Expected and Actual values.',
  ),
);
