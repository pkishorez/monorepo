...
them." The dependency-query transformations (`fileDependencies`,
`folderDependencies`) are that real caller: they operate purely on
`FileGraph` data with no I/O, so they belong in a domain layer rather than
inside `file-cruiser` itself, and a domain layer needs the type to do that.
