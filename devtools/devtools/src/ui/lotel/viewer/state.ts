...
const PersistedSchema = Schema.Struct({
  filters: Schema.Struct({
    ...
    sinceNow: Schema.NullOr(Schema.Number),
    status: Schema.Literals(['all', 'error', 'running']),
  }),
  traceList: Schema.Struct({
    groupBy: Schema.NullOr(Schema.String),
    expandedGroups: StringBooleanRecord,
    selectedTraceId: Schema.NullOr(Schema.String),
  }),
  dock: Schema.Struct({
    open: Schema.Boolean,
    height: Schema.Number,
    sidebarWidth: Schema.Number,
    nameColWidth: Schema.Number,
    sidebarOpen: Schema.Boolean,
    selectedSpanId: Schema.NullOr(Schema.String),
  }),
  palette: Schema.Struct({ open: Schema.Boolean }),
  columnWidths: StringNumberRecord,
  railCollapsed: Schema.Boolean,
  railWidth: Schema.Number,
});

const decodePersisted = Schema.decodeUnknownOption(PersistedSchema);
  ...
  persist(
    ...
    }),
    {
      ...
      merge: (persisted, current) =>
        Option.match(decodePersisted(persisted), {
          onNone: () => current,
          onSome: (decoded) => ({
            ...current,
            ...decoded,
            filters: {
              ...decoded.filters,
              attributeFilters: decoded.filters.attributeFilters.map(
                (filter) => ({ ...filter }),
              ),
            },
            traceList: {
              ...decoded.traceList,
              expandedGroups: { ...decoded.traceList.expandedGroups },
            },
            dock: { ...decoded.dock },
            palette: { ...decoded.palette },
            columnWidths: { ...decoded.columnWidths },
          }),
        }),
    },
  ),
