...
describe('Scope policies', () => {
  ...
  const resolverOf = (currentAuth: CurrentAuthValue) =>
    Layer.succeed(
      Authz.Resolver,
      Authz.Resolver.of({
        resolve: () => Effect.succeed({ currentAuth, refreshedCookies: [] }),
      }),
    );
  const tokenResolver = (scopes: string[]) =>
    resolverOf({ kind: 'token', user, client: { id: 'cli' }, scopes });
  const sessionResolver = resolverOf({
    kind: 'session',
    user: user as User,
    session: { id: 's1' } as Session,
  });
  const Write = Rpc.make('Write', { payload: {}, success: Schema.String }).pipe(
    Authz.guard(Authz.scope('notes:write')),
  );
  const Api = RpcGroup.make(Write);
  const Handlers = Api.toLayer({
    Write: () => Effect.map(Authz.CurrentAuth, (principal) => principal.kind),
  });
  const call = (resolver: ReturnType<typeof resolverOf>) =>
    Effect.gen(function* () {
      const client = yield* RpcTest.makeClient(Api);
      return yield* client.Write(
        {},
  ...
  it('allows a Token Principal carrying the Scope', async () => {
    expect(await Effect.runPromise(call(tokenResolver(['notes:write'])))).toBe(
      'token',
    );
  });

  it('forbids a Token Principal missing the Scope', async () => {
    const error = await Effect.runPromise(
      Effect.flip(call(tokenResolver(['notes:read']))),
    );
    expect(error).toBeInstanceOf(Authz.Forbidden);
  });

  it('forbids a Session Principal, which has no Scopes', async () => {
    const error = await Effect.runPromise(Effect.flip(call(sessionResolver)));
    expect(error).toBeInstanceOf(Authz.Forbidden);
  });
});
