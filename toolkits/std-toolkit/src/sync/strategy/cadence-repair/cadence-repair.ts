...
export type { CadenceConfig } from '../strategy/index.js';
...
const toEntity = <T>(row: CollectionItem<T>): DecodedEntity<T> => {
  ...
  return { value: value as T, meta: _meta! };
};
...
export const runCadenceRepair = <T, E, R = never>(
  deps: CadenceRepairDeps<T, E, R>,
  ...
  Effect.scoped(
    Effect.gen(function* () {
      ...
      const loop: Effect.Effect<void, WriteError, R> = Effect.gen(function* () {
        yield* latch.await;
        ...
        if (invalidU !== undefined) {
          return yield* Effect.fail({
            _tag: 'Invalid',
            reason: `cadence repair stopped: _u is neither a ULID nor an ISO timestamp: ${JSON.stringify(invalidU)}`,
          } as WriteError);
        }
