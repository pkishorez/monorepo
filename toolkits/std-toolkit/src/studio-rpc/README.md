...
A Studio client discovers a table from its snapshot at runtime, so applications never generate table-specific RPC contracts. The group delegates entity lookup and access-pattern queries to the table's existing entity surfaces, preserving their defaults, read migration, tombstone visibility, ordering, and pagination. Results are re-encoded as raw Entities, values in their encoded form, because a Studio client has no application schemas. Physical storage and adapter-native operations stay hidden. The term is defined in [db/CONTEXT.md](../db/CONTEXT.md); the transport decision is [ADR 0009](../../docs/adr/0009-studio-rpc-entity-semantics-encoded-transport.md).
...
- Keys and query conditions are records named by key path (`{ 'board.id': 'work', rank: 3 }`); each value is a string or a number, matching what its path reads, or the call fails with `StudioInvalidInput`.
