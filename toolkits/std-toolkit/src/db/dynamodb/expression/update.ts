 ...
 */
export type CompiledUpdateOperation<T = unknown> = {
...
type SetAppendOperation<
  T,
  Path extends ValidPathsWithCond<T, readonly unknown[]> = ValidPathsWithCond<
    T,
    readonly unknown[]
  >,
> = {
 ...
 * @typeParam T - The entity type this update operates on
 */
export type UpdateOperation<T = unknown> = AnyOperation<T>[];
...
export type UpdateOps<T> = {
  ...
  append: <Path extends ValidPathsWithCond<T, readonly unknown[]>>(
    key: Path,
    value: Get<T, Path>,
  ) => SetAppendOperation<T, Path>;

  /** Prepends values to the beginning of a list attribute */
  prepend: <Path extends ValidPathsWithCond<T, readonly unknown[]>>(
    key: Path,
    value: Get<T, Path>,
...
export function exprUpdate<T>(
  ...
  return builder({
    set: (key, value) => set<T, typeof key>(key, value),
    opAdd: (key: ValidPathsWithCond<T, number>, value: number) =>
      opAdd<T>(key, value),
    opIfNotExists: (key, value) => opIfNotExists<T, typeof key>(key, value),
    append: (key, value) => append<T, typeof key>(key, value),
    prepend: (key, value) => prepend<T, typeof key>(key, value),
  });
}
