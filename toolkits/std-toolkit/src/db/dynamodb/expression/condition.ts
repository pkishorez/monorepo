 ...
 */
export type CompiledConditionOperation<T = unknown> = {
 ...
 * @typeParam T - The entity type this condition operates on
 */
export type CondOperation<T = unknown> = {
  type: 'condition_condition';
  key: ValidPaths<T>;
  op: '=' | '<>' | '<' | '<=' | '>' | '>=';
  value: unknown | FieldRef<T>;
};
 ...
 * @typeParam T - The entity type this condition operates on
 */
export type AndOperation<T = unknown> = {
  type: 'condition_and';
 ...
 * @typeParam T - The entity type this condition operates on
 */
export type OrOperation<T = unknown> = {
  type: 'condition_or';
  conditions: AnyCondition<T>[];
};
 ...
 * @typeParam T - The entity type this condition operates on
 */
export type AttributeNotExistsOperation<T = unknown> = {
 ...
 * @typeParam T - The entity type this condition operates on
 */
export type AttributeExistsOperation<T = unknown> = {
...
export type ConditionOps<T> = {
  ...
  and: (...ops: AnyCondition<T>[]) => AndOperation<T>;

  /** Combines conditions with logical OR */
  or: (...ops: AnyCondition<T>[]) => OrOperation<T>;
...
export function exprCondition<T>(
  ...
  return builder({
    ref: (key) => ref<T>(key as ValidPaths<T>),
    cond: (key, op, value) => ({
      type: 'condition_condition',
      key: key as ValidPaths<T>,
      op,
      value,
    }),
    and: (...ops: AnyCondition<T>[]) => and<T>(...ops),
    or: (...ops: AnyCondition<T>[]) => or<T>(...ops),
    attributeNotExists: (key) => attributeNotExists<T>(key as ValidPaths<T>),
    attributeExists: (key) => attributeExists<T>(key as ValidPaths<T>),
  });
}
    ...
    switch (op.type) {
      case 'condition_condition': {
        ...
        const right =
          ...
          candidate !== null &&
          'type' in candidate &&
          candidate.type === 'field_ref' &&
          'key' in candidate &&
          typeof candidate.key === 'string'
            ? attrBuilder.attr(candidate.key)
            : attrBuilder.value(candidate);
