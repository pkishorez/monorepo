...
const serve = Command.make(
  ...
  Effect.fn(function* ({ port }) {
    console.log(`kai-toolkit listening on http://${PLAYGROUND_HOST}:${port}`);
    yield* Effect.never.pipe(Effect.provide(makePlaygroundServer(port)));
  }),
).pipe(Command.withDescription('Run the KAI Toolkit playground server'));

Command.make('kai-toolkit', {}, () => Effect.void).pipe(
