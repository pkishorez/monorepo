import {
  ...
  formatSpanName as formatSpanNameValue,
  isLog as isLogEvent,
  spanDuration as spanDurationValue,
} from './format';
import { spanTimelineBounds as timelineBounds } from './timeline';
...
export function groupByTrace(spans: OtelSpan[]): TraceGroup[] {
  ...
  return Array.from(byTrace.entries()).map(([traceId, traceSpans]) => {
    ...
    const name = rootNode
      ? formatSpanNameValue(rootNode.span.name, rootNode.span.attributes)
      : traceId;

    // A parent can finish before its children (for example after cancellation).
    // Trace duration covers all spans; individual rows keep their own timing.
    const startTime = minStart;
...
export function formatDuration(
  ...args: Parameters<typeof formatDurationValue>
) {
  return formatDurationValue(...args);
}

export function formatSpanName(
  ...args: Parameters<typeof formatSpanNameValue>
) {
  return formatSpanNameValue(...args);
}

export function isLog(...args: Parameters<typeof isLogEvent>) {
  return isLogEvent(...args);
}

export function spanDuration(...args: Parameters<typeof spanDurationValue>) {
  return spanDurationValue(...args);
}

export function spanTimelineBounds(...args: Parameters<typeof timelineBounds>) {
  return timelineBounds(...args);
}
