...
| `AiRpcLive.layer`   | Layer of `AiRpc` handlers; takes an optional `HarnessHostConfig` and requires the AI Table's `StdTableService<'ai-toolkit'>`.           |
...
| Export          | What it does                                                                             |
| --------------- | ---------------------------------------------------------------------------------------- |
| `aiTable`       | The `StdTable` named `ai-toolkit` with its primary key and GSI1 to GSI5.                 |
...
The `ai-toolkit` binary exposes `ai-toolkit serve --port 3001`, which runs the Playground Server against the current directory.
...
import { defaultBroadcaster } from 'std-toolkit/core';
import { Memory } from 'std-toolkit/db/memory';
import { HarnessHost } from 'ai-toolkit/rpc/live';
import { aiTable, messages, runs, threads } from 'ai-toolkit/table';
...
The docs demo connects over WebSocket, syncs `threads` and `messages` into live collections, and folds rows into a conversation. Trimmed from `apps/docs/src/demos/ai-toolkit/client/client.ts`.
