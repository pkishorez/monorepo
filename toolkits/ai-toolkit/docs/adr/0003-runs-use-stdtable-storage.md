...
Application entities belong to a separate application-owned StdTable. Sharing a physical table was rejected because AI Toolkit must control and evolve its index topology, schemas, retention, and recovery records without constraining or being constrained by application access patterns. The application may still choose where and how each adapter table is deployed, but the two logical tables remain independent.
