...
type ServerRequestHandler = (id: RequestId, request: ServerRequest) => void;
...
export class CodexClient extends Effect.Service<CodexClient>()('CodexClient', {
  scoped: Effect.gen(function* () {
    let nextId = 1;
    let process: ChildProcess | null = null;
    const pending = new Map<
      number,
      Deferred.Deferred<unknown, CodexClientError>
    >();
    ...
    const spawnProcess = () => {
      ...
      rl.on('line', (line) => {
        ...
        if (
          ...
          if (deferred) {
            ...
            Effect.runFork(
              Effect.logWarning(
                'codex: received response for unknown request id',
              ).pipe(Effect.annotateLogs({ requestId: response.id })),
            );
          }
          return;
        }
        ...
        Effect.runFork(
          Effect.logWarning(
            'codex: received unrecognized message from subprocess',
          ).pipe(Effect.annotateLogs({ line })),
        );
      });
      ...
      child.on('exit', (code, signal) => {
        if (pending.size > 0) {
          Effect.runFork(
            Effect.logError(
              'codex: subprocess exited with pending requests',
            ).pipe(
              Effect.annotateLogs({
                exitCode: code ?? 'null',
                signal: signal ?? 'null',
