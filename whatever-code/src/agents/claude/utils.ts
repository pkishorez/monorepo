...
export const appendToQueuedTurn = (
  ...
  Effect.gen(function* () {
    ...
    if (!existing) {
      yield* insertClaudeUserMessage(sessionId, turnId, prompt).pipe(
        Effect.orDie,
      );
      return;
    }
    ...
    const merged = [
      ...promptToBlocks(existingContent),
      ...promptToBlocks(prompt),
    ];
...
function promptToBlocks(prompt: typeof PromptContent.Type): Array<
...
export const readMergedPrompt = (sessionId: string, turnId: string) =>
  Effect.gen(function* () {
    ...
    const userMessages = result.items
      .filter((m) => (m.value.data as any).type === 'user')
      .map(
        (m) =>
          (m.value.data as any).message.content as typeof PromptContent.Type,
      );
