...
[`websocket-rpc-client`](../websocket-rpc-client/README.md) — it re-subscribes your
...
watch: () =>
  Stream.unwrap(
    Effect.gen(function* () {
      ...
      const saved = yield* checkpoint.get(Cursor).pipe(Effect.orDie);
        ...
        Stream.mapEffect(() =>
          readSince(cursor).pipe(
            Effect.tap((batch) => {
              ...
              return checkpoint.put({ cursor }, Cursor).pipe(Effect.orDie);
            }),
          ),
        ),
      );
...
The schema is optional. Structured-cloneable values round-trip as-is:
...
Prefer a schema when the value's shape may change between deploys — see _Limitations_.
...
StreamCheckpoint: Context.Reference<StreamCheckpointService>

interface StreamCheckpointService {
  get: <S>(schema?: S) => Effect<Option<S['Type']>, SchemaError>
  put: <S>(value: S['Type'], schema?: S) => Effect<void, SchemaError>
  clear: Effect<void>
}
```
