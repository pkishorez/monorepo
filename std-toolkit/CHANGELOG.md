...
- Removed `SingleEntityESchema` and `AnySingleEntityESchema`. Define singleton values with `ESchema.make(...)`, then select singleton behavior with `table.singleEntity(schema)` or `singleItemSync`. Generic singleton helpers can use `AnyUnkeyedESchema` to reject keyed entities. Standalone snapshots of these schemas now use the `struct` kind; table snapshots continue to classify their binding as `singleton`.
