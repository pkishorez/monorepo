...
describe('approve command', () => {
  it('approves a new latest version by creating its snapshot and manifest hash', async () => {
    const root = copySchemaSnapshotFixture('new-version', 'eschema-approval-');
    ...
    expect(readText(join(schemaRoot, '__snapshots__', 'v1.ts.snap'))).toBe(v1);
    expect(JSON.parse(readText(join(schemaRoot, 'snapshots.json')))).toEqual({
  ...
  it('ignores a new version without writing snapshot files', async () => {
    ...
    expect(existsSync(join(schemaRoot, '__snapshots__', 'v1.ts.snap'))).toBe(
      false,
    );
  ...
  it('shows a diff and approves a modified latest version', async () => {
    ...
    expect(JSON.parse(readText(join(schemaRoot, 'snapshots.json'))).v2).toBe(
      hashSnapshotContent(updatedV2),
    );
    expect((await analyze(root)).issues).toEqual([]);
  });
  ...
  it('approves a modified non-latest version when forced', async () => {
    ...
    expect(JSON.parse(readText(join(schemaRoot, 'snapshots.json'))).v1).toBe(
      hashSnapshotContent(updatedV1),
    );
    expect((await analyze(root)).issues).toEqual([]);
  });
  ...
  it('reports deleted version references without writing approvals', async () => {
    ...
    const schemaRoot = join(root, 'user');
    const deletedV2 = readText(join(schemaRoot, '__snapshots__', 'v2.ts.snap'));
