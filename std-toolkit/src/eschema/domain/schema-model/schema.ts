...
export function schemaDescriptor(schema: Schema.Top): ESchemaDescriptor {
  const { schema: root, definitions } = Schema.toJsonSchemaDocument(schema);
  const value = { ...root } as ESchemaDescriptor;
  if (Object.keys(definitions).length > 0) value.$defs = definitions;
  return value;
}
