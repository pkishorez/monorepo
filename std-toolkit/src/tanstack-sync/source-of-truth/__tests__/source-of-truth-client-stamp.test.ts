...
describe('source-of-truth: client _c stamp', () => {
  ...
  itEffect('re-stamps _c on a subsequent write of the same entity', () =>
    Effect.gen(function* () {
      ...
      yield* sot.write([entity('a', 'Alpha v2', '2024-01-02T00:00:00.000Z')]);
  ...
  itEffect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* sot.write([entity('a', 'Alpha', '2024-01-02T00:00:00.000Z')]);
...
describe('source-of-truth: _s reconciliation on skip', () => {
  itEffect('reconciles _s onto a stored record skipped by _u', () =>
    Effect.gen(function* () {
      ...
      const { upserts } = yield* sot.write([
        entityWithS('a', 'Alpha', '2024-01-01T00:00:00.000Z', 1_700_000_000),
      ]);
  ...
  itEffect('does not reconcile when incoming _s matches the stored _s', () =>
    Effect.gen(function* () {
      const storage = memoryOfflineStorage();
      const group = storage.group('sot/items');
      const sot = makeSourceOfTruth<Item>({ schema, group });
