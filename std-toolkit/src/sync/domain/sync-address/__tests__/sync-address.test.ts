...
import { serializePartition } from '../../partition-identity/index.js';
import {
...
describe('sync addresses', () => {
  it.each([
    ...
    expect(normalizeSyncName(input)).toBe(expected);
  });

  it.each(['', ' --- ', '́̈', '🚀'])(
    'rejects empty normalized name %j',
    (name) => {
      expect(() => normalizeSyncName(name)).toThrow(
  ...
  it('builds qualified collection names and readable addresses', () => {
    ...
    expect(qualifyCollectionName('a', 'B')).toBe('a.b');
  ...
  it('does not make lossy addresses the exact partition identity', () => {
    ...
    expect(serializePartition(numeric)).not.toBe(serializePartition(textual));
  });
});
