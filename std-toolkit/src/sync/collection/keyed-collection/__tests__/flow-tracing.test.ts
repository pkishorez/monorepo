...
describe('collection flow tracing', () => {
  ...
  it('records one collection flow with stable global and partition lanes', async () => {
    ...
    expect(
      flow.activations
        .filter(({ name }) => name !== 'Std Sync lifecycle')
        .every(({ endItemId }) => endItemId !== null),
    ).toBe(true);
    expect(
    ...
    expect(participants).toEqual(
      new Set([
        'browser:alice',
        'browser:alice/comment',
        'browser:alice/comment/{global}.global-worker',
        'browser:alice/comment/{postid=post-1}.partition-worker',
      ]),
    );
    expect(
      flow.items.filter(
  ...
  it('keeps one active flow across repeated collection starts and cleanups', async () => {
    ...
    expect(flows[0]?.activations).toMatchObject([
      { participantName: 'comments', outcome: null },
