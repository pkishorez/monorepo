...
import type { IdbKey, IdbRangeSpec, IdbRecord } from '../idb-database/index.js';
import type {
  AnyEntityESchema,
  AnyUnkeyedESchema,
  ...
  make(storeName: string) {
    return {
      ...
      primary<Pk extends string, Sk extends string>(pk: Pk, sk: Sk) {
        return new IdbTableBuilder(storeName, { pk, sk }, {});
      },
    };
  },
};
...
const buildRange = (cond: KeyConditionParameters): IdbRangeSpec => {
  ...
  if ('beginsWith' in skCondition) {
    return { pk: cond.pk, prefix: skCondition.beginsWith };
  }
...
>(
  ...
  const rawQuery = (
    ...
    return Effect.gen(function* () {
      ...
      const items = yield* db.getRange(storeName, index, range, {
        direction,
  ...
  return {
    ...
    setup(): Effect.Effect<void, IdbDBError, IdbDB> {
      return IdbDB.pipe(
        Effect.flatMap((db) => db.setup(storeName, secondaryIndexMap)),
      );
    },
    ...
    getItem(
      ...
      return Effect.gen(function* () {
        const db = yield* IdbDB;
        const record = yield* db.get(storeName, key);
        return { Item: record };
      });
    },
    ...
    putItem(record: IdbRecord): Effect.Effect<void, IdbDBError, IdbDB> {
      return IdbDB.pipe(Effect.flatMap((db) => db.put(storeName, record)));
    },
    ...
    hardDeleteItem(key: IdbKey): Effect.Effect<void, IdbDBError, IdbDB> {
      return IdbDB.pipe(Effect.flatMap((db) => db.delete(storeName, key)));
    },
    ...
    dangerouslyRemoveEntityItems(
      ...
      return IdbDB.pipe(
        Effect.flatMap((db) => db.deleteEntity(storeName, entityName)),
    ...
    dangerouslyRemoveAllItems(
      ...
      return IdbDB.pipe(
        Effect.flatMap((db) => db.clear(storeName)),
        Effect.map(({ rowsDeleted }) => ({ itemsDeleted: rowsDeleted })),
      );
    },
  };
...
>(base: IdbTableInstance<TPrimaryIndex, TSecondaryIndexMap>) {
  ...
  return {
    ...
    entity<TS extends AnyEntityESchema>(eschema: TS) {
      return makeIdbEntity(base, register).eschema(eschema);
    },
    ...
    singleEntity<TS extends AnyUnkeyedESchema>(eschema: TS) {
      return makeIdbSingleEntity(base, register).eschema(eschema);
    },
     ...
     * Ops built against a different table are rejected with a typed error.
     *
    ...
    transact(
      ...
      return Effect.gen(function* () {
        ...
        for (const op of ops) {
          if (op.table !== base) {
            return yield* Effect.fail(
              IdbDBError.foreignTransactionItem(op.entityName, {
                tableName: base.storeName,
                partitionKey: op.pk,
                sortKey: op.sk,
        ...
        for (const { count, pk, sk } of keyCounts.values()) {
          if (count > 1) {
            return yield* Effect.fail(
              IdbDBError.duplicateTransactionTarget(pk, sk, {
                tableName: base.storeName,
              }),
            );
          }
