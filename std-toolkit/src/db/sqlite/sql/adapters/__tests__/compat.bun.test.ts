    ...
    describe('Compatibility', () => {
      ...
      describe('better-sqlite3 writes, bun reads', () => {
        ...
        test('bun can open the database file', () => {
          const tables = db
            .prepare(
              "SELECT name FROM sqlite_master WHERE type='table' AND name='compat_test'",
            )
            .all();
          ...
          const row = Effect.runSync(
            Effect.gen(function* () {
              ...
              return yield* sqliteDB.get<{
                ...
                name: string;
                age: number;
              }>('compat_test', Sql.where('id', '=', 'bs3-2'));
            }).pipe(Effect.provide(layer)),
          );
        ...
        test('bun sees indexes created by better-sqlite3', () => {
          const indexes = db
            .prepare(
              "SELECT name FROM sqlite_master WHERE type='index' AND name='idx_compat_name'",
            )
            .all();
        ...
        test('column types are preserved across adapters', () => {
          const row = db
            .prepare('SELECT typeof(age) as t FROM compat_test LIMIT 1')
            .get() as { t: string };
      ...
      describe('bun writes, better-sqlite3 reads', () => {
        ...
        beforeAll(() => {
          ...
          Effect.runSync(
            Effect.gen(function* () {
              ...
              yield* sqliteDB.createIndex('compat_test', 'idx_compat_name', [
                'name',
              ]);
        ...
        test('better-sqlite3 reads rows written by bun', () => {
          const result = spawnSync(
            ...
            [
              'tsx',
              join(testDir, 'compat-read-better-sqlite3.ts'),
              dbPath,
              'query',
            ],
            { stdio: 'pipe', encoding: 'utf-8', timeout: 30_000 },
          ...
          const rows = JSON.parse(result.stdout.trim()) as {
            id: string;
            name: string;
            age: number;
          }[];
          expect(rows).toHaveLength(3);
          expect(rows.find((r) => r.id === 'bun-1')).toMatchObject({
            name: 'Dave',
            age: 28,
          });
          expect(rows.find((r) => r.id === 'bun-2')).toMatchObject({
            name: 'Eve',
            age: 32,
          });
          expect(rows.find((r) => r.id === 'bun-3')).toMatchObject({
            name: 'Frank',
            age: 40,
          });
        });
