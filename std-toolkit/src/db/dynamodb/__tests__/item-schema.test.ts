...
describe('DynamoDB item schema', () => {
  it('uses top-level metadata and configured index attribute names', () => {
    const decoded = Schema.decodeSync(schema)(item);

    expect(unmarshall(decoded)).toEqual({
      PK: item.pk,
      SK: item.sk,
      _e: item.meta._e,
      _v: item.data._v,
    ...
    expect(Schema.encodeSync(schema)(decoded)).toEqual(item);
  });

  it('fails with a parse error on an item missing its key attributes', async () => {
    const result = await Effect.runPromise(
      Effect.result(Schema.encodeEffect(schema)({ _e: { S: 'Person' } })),
    );
