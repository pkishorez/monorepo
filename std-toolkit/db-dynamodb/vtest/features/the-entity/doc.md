...
A `DynamoEntity` is how you actually read and write a _kind_ of item on the
...
- `insert(value)` — validate, key, store; fails if the item already exists
- `get(key)` — fetch by primary key fields, or `null`
- `update(key, { update })` — merge a patch onto an existing item
...
`delete(key)` does **not** remove the item. It flips `_d` to `true` and bumps
`_u`, so a later `get` still returns the item but marked deleted, and sync can
observe the deletion as just another change. This is what makes the store safe
to replicate: deletions are events, not gaps. Deleting an item that was never
written fails with `NoItemToDelete` — there is nothing to mark.
