...
The primary index decides _where_ a record lives and _how_ you query a group of
them. Two halves: the **partition key** (`pk`) gathers related records together,
...
SQLiteEntity.make(table).eschema(UserSchema).primary();
...
on a `Post` keyed by `postId` stores each post at
`pk = "Post#<authorId>"`, `sk = <postId>` — every post for one author sits in
one partition, sorted by post id.
...
a partition by its key fields and walk its sort key. There is no ad-hoc `WHERE`.
