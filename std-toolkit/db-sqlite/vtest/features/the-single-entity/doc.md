...
cursor. Modelling that as a keyed collection forces a fake id like `"the-only-
one"`. A `SQLiteSingleEntity` is the cleaner drawer: exactly one record, no id,
and a **mandatory default** so reads never come back empty.
...
the empty string — that empty `_u` is how you tell "never written" from "written".

## get / put / update

The surface is three operations and none take a key:

- `get()` — the record, or the default; always present
- `put(value)` — write unconditionally (upsert)
- `update({ update })` — patch an existing record; **fails** if nothing was ever
  written (there's no record to merge onto)
