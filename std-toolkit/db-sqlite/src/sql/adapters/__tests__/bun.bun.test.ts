...
describe("SqliteDBBun adapter", () => {
  ...
  describe("createTable", () => {
    ...
    test("creates table with composite primary key", async () => {
      ...
      const tables = db
        .prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='composite_pk'")
        .all();
      expect(tables).toHaveLength(1);
    });
  });
  ...
  describe("createIndex", () => {
    test("creates an index on columns", async () => {
      ...
      const indexes = db
        .prepare("SELECT name FROM sqlite_master WHERE type='index' AND name='idx_test_name'")
        .all();
  ...
  describe("get", () => {
    ...
    test("fails when row not found", async () => {
      const result = await Effect.runPromise(
        Effect.gen(function* () {
          const sqliteDB = yield* SqliteDB;
          return yield* sqliteDB.get("test_table", Sql.where("id", "=", "missing"));
        }).pipe(
  ...
  describe("query", () => {
    beforeEach(async () => {
      ...
      db.prepare("INSERT INTO test_table (id, name, age) VALUES (?, ?, ?)").run("a", "Alice", 25);
      db.prepare("INSERT INTO test_table (id, name, age) VALUES (?, ?, ?)").run("b", "Bob", 30);
      db.prepare("INSERT INTO test_table (id, name, age) VALUES (?, ?, ?)").run("c", "Charlie", 35);
    });
    ...
    test("commit persists changes", async () => {
      await run(
        Effect.gen(function* () {
          ...
          yield* sqliteDB.insert("test_table", { id: "tx-1", name: "TxUser", age: 40 });
          yield* sqliteDB.commit();
        }),
      );

      const rows = db.prepare("SELECT * FROM test_table WHERE id = 'tx-1'").all();
    ...
    test("rollback discards changes", async () => {
      await run(
        Effect.gen(function* () {
          const sqliteDB = yield* SqliteDB;
          yield* sqliteDB.begin();
          yield* sqliteDB.insert("test_table", { id: "tx-2", name: "RollbackUser", age: 50 });
          yield* sqliteDB.rollback();
        }),
      );

      const rows = db.prepare("SELECT * FROM test_table WHERE id = 'tx-2'").all();
      expect(rows).toHaveLength(0);
    });
  });
});
