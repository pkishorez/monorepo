...
if (!dbPath) {
  console.error("Usage: compat-read-better-sqlite3.ts <db-path> [query|get|update]");
  process.exit(1);
}
...
if (mode === "update") {
  db.prepare("UPDATE compat_test SET name = ?, age = ? WHERE id = ?").run("Updated", 99, "rt-1");
  db.prepare("INSERT INTO compat_test (id, name, age) VALUES (?, ?, ?)").run("rt-2", "NewRow", 50);
  ...
  const rows = Effect.runSync(
    Effect.gen(function* () {
      ...
      return yield* sqliteDB.query("compat_test", { clause: "1=1", params: [] });
