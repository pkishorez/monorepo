...
export const adapterNames = ['dynamodb', 'idb', 'memory', 'sqlite'] as const;

let sessionNumber = 0;
const uniqueName = () => `std-table-stories-${process.pid}-${++sessionNumber}`;

export const sequentialUlid = () => {
  let issued = 0;
  return () => String(++issued).padStart(26, '0');
};

export const onDynamoDB = <A, E, R>(program: Effect.Effect<A, E, R>) =>
  Effect.gen(function* () {
...
export const onIDB = <A, E, R>(program: Effect.Effect<A, E, R>) =>
  Effect.gen(function* () {
...
export const onSQLite = <A, E, R>(program: Effect.Effect<A, E, R>) =>
  Effect.gen(function* () {
...
export const onMemory = <A, E, R>(program: Effect.Effect<A, E, R>) => {
  const configured = Memory.make(table);
  return program.pipe(
    Effect.provide(configured.layer),
    Effect.provideService(Ulid, sequentialUlid()),
  );
...
export const agree = <A>(results: {
  ...
  JSON.stringify(results.idb) === JSON.stringify(results.memory) &&
  JSON.stringify(results.memory) === JSON.stringify(results.sqlite);

type TableLayer = MemoryTable<typeof table.logicalName>['layer'];

type LayerScenario<A, E> = (
  outer: TableLayer,
  inner: TableLayer,
) => Effect.Effect<A, E>;
...
const onDynamoDBLayers = <A, E>(scenario: LayerScenario<A, E>) => {
  const make = () =>
    DynamoDB.make(table, {
      tableName: uniqueName(),
      region: 'local',
  ...
  return Effect.gen(function* () {
    yield* outer.setup;
    yield* inner.setup;
    return yield* scenario(outer.layer, inner.layer).pipe(
      Effect.provideService(Ulid, sequentialUlid()),
    );
  }).pipe(
    Effect.ensuring(
      Effect.all([outer.teardown, inner.teardown]).pipe(Effect.orDie),
    ),
  );
};

const onIDBLayers = <A, E>(scenario: LayerScenario<A, E>) => {
  const indexedDB = new IDBFactory();
  const outerName = uniqueName();
  const innerName = uniqueName();
  const outerDatabase = IDB.database({ databaseName: outerName, indexedDB });
  const innerDatabase = IDB.database({ databaseName: innerName, indexedDB });
  const outer = IDB.make(table, { database: outerDatabase });
  const inner = IDB.make(table, { database: innerDatabase });
  return Effect.gen(function* () {
    yield* outer.setup;
    yield* inner.setup;
    return yield* scenario(outer.layer, inner.layer).pipe(
    ...
    Effect.ensuring(
      Effect.all([
        deleteIDB(indexedDB, outerName, outerDatabase),
        deleteIDB(indexedDB, innerName, innerDatabase),
      ]),
    ),
  );
};

const onMemoryLayers = <A, E>(scenario: LayerScenario<A, E>) =>
  scenario(Memory.make(table).layer, Memory.make(table).layer).pipe(
    Effect.provideService(Ulid, sequentialUlid()),
  );

const onSQLiteLayers = <A, E>(scenario: LayerScenario<A, E>) => {
  const outerDatabase = makeNodeSQLite({ path: ':memory:' });
  const innerDatabase = makeNodeSQLite({ path: ':memory:' });
  const outer = SQLite.make(table, { database: outerDatabase });
  const inner = SQLite.make(table, { database: innerDatabase });
  return Effect.gen(function* () {
    yield* outer.setup;
    yield* inner.setup;
    return yield* scenario(outer.layer, inner.layer).pipe(
...
export const layerParity = <A, E>(scenario: LayerScenario<A, E>) =>
  Effect.all({
    dynamodb: onDynamoDBLayers(scenario),
    idb: onIDBLayers(scenario),
    memory: onMemoryLayers(scenario),
    sqlite: onSQLiteLayers(scenario),
  });
