...
describe('keyed sync offline storage', () => {
  ...
  it('keeps newer persisted tombstones over older live server entities', async () => {
    ...
    await Effect.runPromise(
      storage
        ...
        .put(
          'todo-1',
          tombstone({ id: 'todo-1', listId: 'inbox', title: 'deleted' }, '2'),
        ),
    );
  ...
  it('resumes a partition strategy from persisted sync state', async () => {
    ...
    await vi.waitFor(() => expect(observed).toEqual([{ cursor: 'page-2' }]));
  ...
  it('persists registry writes while unmounted and projects them on mount', async () => {
    ...
    std.registry().process({
      persist: true,
      values: [
        entity({ id: 'todo-1', listId: 'inbox', title: 'from registry' }, '1'),
      ],
    });
...
describe('single-item sync offline storage', () => {
