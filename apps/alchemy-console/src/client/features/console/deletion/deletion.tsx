...
import { useDeletionPreview, DeletionPreview } from './preview.tsx';
...
function DeletionDialog({
  ...
  const { plan } = preview;
  // Resources the user chose to ignore: Alchemy drops their state on confirm
  ...
  // Every blocker is either resolved by the server or a row the user chose to ignore.
  const confirmable =
    !!plan &&
    (plan.executable ||
      plan.resources.every(
        (resource) => resource.readiness === 'ready' || ignored(resource),
      ));
