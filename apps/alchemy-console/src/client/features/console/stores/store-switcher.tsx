...
export function StoreSwitcher({
  ...
  SettingsLink: ComponentType<LinkProps>;
  onStoreCreated: (storeId: string) => void;
  stores: readonly (typeof storeView.Type)[];
  adding: boolean;
  setAdding: (adding: boolean) => void;
}) {
  const current = stores.find((store) => store.id === storeId);
  const credentials = useCredentials();
  const state = credentials.data?.find(
    (item) => item.id === current?.state.credentialId,
  );
  return (
    <>
      <DropdownMenu>
        ...
        >
          ...
          <span className="min-w-0 flex-1">
            ...
            <span className="block truncate text-xs text-muted-foreground">
              {state ? describeCredential(state) : 'Store'}
            </span>
          </span>
        ...
        >
          ...
          <DropdownMenuItem render={<SettingsLink />}>
            <KeyRound />
            Credentials
          </DropdownMenuItem>
        </DropdownMenuContent>
