...
const workersSubdomain = Schema.Struct({ subdomain: nonEmpty });

/** Confirms the token can read the account's Workers subdomain, which every Console token needs. */
export const verify = Effect.fn('CloudflareProvider.verify')(function* (
  ...
  yield* describeFailure(
    ...
    readJson(
      apiResult(workersSubdomain),
      HttpClientRequest.get(
        `https://api.cloudflare.com/client/v4/accounts/${account.accountId}/workers/subdomain`,
      ).pipe(HttpClientRequest.bearerToken(account.apiToken)),
      secrets,
    ),
  );
  return { account: account.accountId.toLowerCase() };
});
...
export const discover = Effect.fn('CloudflareProvider.discover')(
  function* (account: CloudflareAccount) {
    ...
    const { subdomain } = yield* describeFailure(
      ...
      read('/workers/subdomain', workersSubdomain).pipe(
        Effect.filterOrFail(
