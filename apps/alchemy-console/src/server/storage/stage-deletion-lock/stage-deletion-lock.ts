...
export const makeStageDeletionLock = (database: SQLiteDriver) => ({
  setup: database.run(
  ...
  layer: Layer.succeed(StageDeletionLock, {
    acquire: (key, owner) =>
      database
        .run(
