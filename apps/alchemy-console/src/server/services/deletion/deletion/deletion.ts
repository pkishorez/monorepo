...
import { deletionRequest } from './request.ts';

type Target = typeof deletionRequest.Type;
const failure = (reason: string) =>
  new DeletionError({ code: 'remote-error', reason });
const validate = Schema.decodeUnknownEffect(deletionRequest);
...
export const preview = (target: Target) =>
  Stream.callback<typeof previewEvent.Type, DeletionError>((queue) =>
    Effect.gen(function* () {
...
export const destroy = (target: Target) =>
  Stream.callback<typeof deletionEvent.Type, DeletionError>((queue) =>
    Effect.gen(function* () {
      const input = yield* validate(target).pipe(
        Effect.mapError(() =>
          failure(
