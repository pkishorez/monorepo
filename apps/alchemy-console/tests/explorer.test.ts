...
it('loads each level independently using the saved connection, with masked state and outputs', async () => {
  ...
  await run(
    ...
    (client) =>
      Effect.gen(function* () {
        const stacks = yield* client['Explorer.ListStacks'](
          { storeId: 'store' },
          options(),
        );
        ...
        const stages = yield* client['Explorer.ListStages'](
        ...
        const resources = yield* client['Explorer.ListResources'](
          input,
          options(),
        );
        expect(resources.data).toEqual(['Bucket', 'Gone', 'Task']);
        expect(fetch).toHaveBeenCalledTimes(3);
        const bucket = yield* client['Explorer.GetResourceState'](
        ...
        const task = yield* client['Explorer.GetResourceState'](
          { ...input, resource: 'Task' },
          options(),
        );
        ...
        const gone = yield* client['Explorer.GetResourceState'](
          { ...input, resource: 'Gone' },
          options(),
        );
        expect(gone.data).toBeNull();
        const stageView = yield* client['Explorer.GetStageView'](
          input,
          options(),
        );
        for (const secret of [
  ...
  await run(fetch, (client) =>
    Effect.gen(function* () {
      for (const [user, storeId, tag] of [
        ['bob', 'store', 'BrowseError'],
        ['alice', 'missing', 'BrowseError'],
        ...
        const calls = [
          client['Explorer.ListStacks'](input, options(user)).pipe(
            Effect.asVoid,
          ),
          client['Explorer.ListStages'](input, options(user)).pipe(
            Effect.asVoid,
          ),
          client['Explorer.ListResources'](input, options(user)).pipe(
            Effect.asVoid,
          ),
          client['Explorer.GetStageView'](input, options(user)).pipe(
            Effect.asVoid,
          ),
          client['Explorer.GetResourceState'](input, options(user)).pipe(
            Effect.asVoid,
          ),
        ];
...
it('rejects malformed resource state instead of returning a partial dump', async () => {
  ...
  const error = await run(
    ...
    (client) =>
      Effect.flip(
        client['Explorer.GetResourceState'](
...
it('returns safe remote errors without exposing token or upstream bodies', async () => {
  ...
  const error = await run(fetch, (client) =>
    Effect.flip(client['Explorer.ListStacks']({ storeId: 'store' }, options())),
  );
...
it('rejects non-Worker endpoints before sending credentials', async () => {
  ...
  const error = await run(
    ...
    (client) =>
      Effect.flip(
        client['Explorer.ListStacks']({ storeId: 'store' }, options()),
      ),
    'https://127.0.0.1',
  );
