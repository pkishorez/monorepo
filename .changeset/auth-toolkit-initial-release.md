...
- `server/http-api`: `withAuthz()` to protect Effect HTTP API endpoints and groups, provide typed `CurrentAuth` to handlers, enforce application-owned authorization policies, relay refreshed cookies, and distinguish unauthenticated, forbidden, and unavailable-verifier responses as HTTP 401, 403, and 503.
